import { FunctionalComponent, h, inject, ref, Ref, VNode } from 'vue';
...
const getElementClasses = (
  ...
  return [...Array.from(ref.value?.classList || []), ...defaultClasses].filter(
    (c: string, i, self) => !componentClasses.includes(c) && self.indexOf(c) === i,
  );
};
...
export const defineContainer = <Props extends object>(
  ...
  const Container: FunctionalComponent<Props & InputProps> = (props, { attrs, slots, emit }) => {
    ...
    return h(name, finalProps, slots.default && slots.default());
  };
